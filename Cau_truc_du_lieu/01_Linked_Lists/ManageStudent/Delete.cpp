...
Dựa vào template, hãy hoàn thành chương trình xóa tất cả sinh viên có họ và tên X (X được nhập vào).
...
Dòng đầu tiên: Họ tên sinh viên cần xóa.
Dòng thứ 2: Số lượng n phần tử trong danh sách.
...
OUTPUT
Hiển thị Danh sách sinh viên sau khi thực hiện thao tác xóa trên dòng liên tiếp. Mỗi dòng hiển thị 8 thông tin theo thứ tự sau:
...
Nếu danh sách rỗng thì in ra "Empty List."*/
...
bool Xoa_TheoHoTen(LIST &l, string hoten) {
    if (l.pHead == NULL) {
        return false;
    }

...
Dựa vào template, hãy hoàn thành chương trình giúp sắp xếp danh sách sinh viên giảm dần theo MSSV.
...
OUTPUT
Hiển thị Danh sách n sinh viên trên n dòng liên tiếp. Mỗi dòng hiển thị 8 thông tin theo thứ tự sau:
...
void SapXep(LIST &L){
    ...
    // Bubble sort - O(n^2) nhưng đơn giản và ổn định
    bool swapped;
    NODE *p, *q;
    ...
    do {
        ...
        while (p->pNext != NULL) {
            if (p->key.MASV < p->pNext->key.MASV) { // Sắp xếp GIẢM dần
                // Swap data
                SINHVIEN temp = p->key;
                p->key = p->pNext->key;
                p->pNext->key = temp;
